    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-xs">
        <motion.div
          ...
          className="bg-white w-full max-w-lg rounded-3xl border border-slate-200 shadow-2xl overflow-hidden relative"
        >
          {/* Header */}
          <div className="p-6 border-b border-slate-100 flex items-start justify-between bg-slate-50/50">
            <div>
              <span className="text-[11px] font-semibold uppercase tracking-wider text-brand-600 bg-brand-50 px-2.5 py-0.5 rounded-full border border-brand-200/70">
                Store Analytics
              </span>
              ...
              <>
                ...
                <div className="flex items-center justify-between p-4.5 rounded-2xl bg-slate-50 border border-slate-200/80">
                  ...
                  <div className="text-right">
                    <span className="text-xs text-slate-400 block mb-1">Your review</span>
                    <StarRating
                      ...
                      size="w-5 h-5"
                    />
                    {savingRating && <p className="text-[10px] text-brand-600 mt-1">Updating...</p>}
                  </div>
                </div>
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center justify-between text-xs font-semibold uppercase tracking-wider text-slate-500">
                    <span className="flex items-center gap-1.5">
                      <BarChart2 className="w-4 h-4 text-brand-600" /> Rating Distribution
                    </span>
                    <span>Count</span>
                  </div>
                  ...
                  {[5, 4, 3, 2, 1].map((star) => {
                    ...
                    return (
                      <div key={star} className="flex items-center gap-3 text-xs">
                        <span className="w-6 font-medium text-slate-700 flex items-center gap-0.5">
