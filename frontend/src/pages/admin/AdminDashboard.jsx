...
const ROLE_BADGES = {
  admin: { label: 'Admin', icon: Shield, class: 'bg-admin-50 text-admin-700 border-admin-200' },
  owner: { label: 'Owner', icon: Briefcase, class: 'bg-owner-50 text-owner-700 border-owner-200' },
  user: { label: 'Reviewer', icon: User, class: 'bg-user-50 text-user-700 border-user-200' },
};
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full space-y-8 flex-1">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to="/admin/stores/new"
              className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white hover:bg-slate-50 border border-slate-300 text-slate-700 text-sm font-semibold shadow-xs transition-colors"
            >
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              <Link
                to="/admin/users"
                className="bg-white border border-slate-200/90 rounded-2xl p-6 hover:shadow-md hover:border-brand-300 transition-all group"
              >
                ...
                <p className="font-heading text-3xl sm:text-4xl font-bold text-slate-900 mt-3">
                  {stats.totalUsers}
                </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-7 bg-white rounded-2xl border border-slate-200 p-6 space-y-4 shadow-xs">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  ...
                  <Link to="/admin/stores" className="text-xs font-semibold text-brand-600 hover:underline">
                    View all
                  </Link>
                </div>
                ...
                <div className="space-y-3">
                  {stats.topStores && stats.topStores.length > 0 ? (
                    ...
                    <p className="text-xs text-slate-400 py-4 text-center">No rated stores yet.</p>
                  )}
                </div>
              </div>
                  ...
                  {[5, 4, 3, 2, 1].map((star) => {
                    ...
                    return (
                      <div key={star} className="flex items-center gap-3 text-xs">
                        <span className="w-7 font-medium text-slate-700 flex items-center gap-1">
                    ...
                    return (
                      <div key={u.id} className="py-3 flex items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3 shrink-0">
                          ...
                          <Link
                            to={`/admin/users/${u.id}`}
                            className="text-xs font-semibold text-brand-600 hover:underline hidden sm:inline"
                          >
                            Inspect
                          </Link>
                        </div>
