    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 px-4 py-12">
      <div className="w-full max-w-lg bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold text-brand-700">StoreRate</h1>
          <p className="text-slate-500 text-sm mt-1">Log in to your account to continue</p>
        </div>

        {error && (
          <div className="mb-4 rounded-lg bg-red-50 text-red-700 text-sm px-3.5 py-2.5 border border-red-200">
            {error}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full rounded-lg border border-slate-300 px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 transition-colors"
            />
          </div>
          <div>
          ...
          <button
            ...
            className="w-full bg-brand-600 hover:bg-brand-700 disabled:opacity-60 text-white font-medium py-2.5 rounded-lg text-sm transition-colors shadow-sm"
          >
              ...
              {DEMO_ACCOUNTS.map((demo) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`w-full text-left p-3 rounded-xl border border-slate-200 bg-slate-50/70 hover:bg-white ${demo.hoverBorder} transition-all shadow-xs flex items-start gap-3 group disabled:opacity-50`}
                  >
                    <div className={`p-2 rounded-lg border ${demo.badgeColor} mt-0.5 shrink-0`}>
                      {isLoggingIn ? (
